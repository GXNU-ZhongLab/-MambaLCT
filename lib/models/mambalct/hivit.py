...
from lib.models.mambalct.hivit_backbone import BaseBackbone
...
class Attention(nn.Module):
    ...
    def forward(self, x, rpe_index=None, mask=None,return_att=False):
        B, N, C = x.shape
...
class BlockWithRPE(nn.Module):
    ...
    def forward(self, x, rpe_index=None, mask=None):
        ...
        if self.attn is not None:
            feat, att = self.attn(self.norm1(x), rpe_index, mask,return_att=True)
...
def _create_hivision_transformer(pretrained=False, default_cfg=None, **kwargs):
...
def hivit_base(pretrained=False, **kwargs):
    ...
    model = _create_hivision_transformer(pretrained=pretrained, **model_kwargs)
